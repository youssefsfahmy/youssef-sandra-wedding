...
const SubmissionViewPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    getPartyByConfirmationCode(confirmationCode)
      .then((data) => {
        if (!data?.confirmationCode) {
          setError(
            "We couldn't find that RSVP. Please check your link and try again.",
          );
        } else {
          setParty(data);
        }
  ...
  const handleChangeRSVP = async () => {
    if (!party?.id) return;
    // Route to form with edit flag - it will load existing data for editing
    router.push(`/form?partyId=${party.id}&edit=true`);
  };
          ...
          {party.message && (
            <div
              style={{
                marginBottom: 20,
                padding: "16px 20px",
                borderLeft: `3px solid rgba(88,103,74,0.3)`,
              }}
            >
              <p
                style={{
